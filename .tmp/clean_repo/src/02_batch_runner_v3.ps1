...
# ── Python AST helper init ────────────────────────────────────────────────────
$pyAstHelper = Join-Path $EngineRoot 'src\py_ast_helper.py'
$pythonExe = $null
...
$script:astUsedCount = 0
$script:regexFallbackCount = 0

function Test-StructuralExemption {
    param([string]$FilePath)

    $name = [System.IO.Path]::GetFileName($FilePath)
    $exemptNames = @(
        '__init__.py', 'conftest.py', 'setup.py', 'manage.py', '__main__.py',
        'main.py', 'app.py', 'index.js', 'index.ts', 'index.jsx', 'index.tsx'
    )
    if ($exemptNames -contains $name) { return $true }
    $normalized = $FilePath.Replace('\', '/')
    if ($normalized -match '(^|/)tests?/') { return $true }
    return $false
}

function Get-PythonASTImports {
    param([string]$FullPath)

    if (-not $pythonExe -or -not (Test-Path -LiteralPath $pyAstHelper)) {
        return $null
    }
    try {
        $output = & $pythonExe $pyAstHelper $FullPath 2>$null
        if ($LASTEXITCODE -eq 0 -and $output) {
            $result = $output | ConvertFrom-Json
            if ($result.parser -eq 'ast') {
                $script:astUsedCount++
                return $result
            }
        }
...
function Get-DISignals {
    param([string]$Text)

    $signals = New-Object System.Collections.Generic.List[object]
    $diPatterns = @(
        @{ pattern = 'Depends\s*\('; confidence = 90; evidence = 'fastapi_depends' }
        @{ pattern = '(?:db|session)\s*:\s*Session|Session\s*=\s*Depends\s*\('; confidence = 85; evidence = 'sqlalchemy_session_inject' }
        @{ pattern = 'importlib\.import_module\s*\('; confidence = 80; evidence = 'importlib_import_module' }
        @{ pattern = 'app\.dependency_overrides|inject\s*\(|wire\s*\(|Provide\s*\(|providers\s*='; confidence = 75; evidence = 'di_framework_pattern' }
        @{ pattern = 'get_db\s*\(\s*\)|get_session\s*\(\s*\)'; confidence = 70; evidence = 'session_factory_inject' }
    )
    foreach ($entry in $diPatterns) {
...
function Analyze-File {
    ...
    $isStructuralExempt = Test-StructuralExemption -FilePath $Entry.file
    $diSignals = Get-DISignals -Text $text
    $isDIWired = $diSignals.Count -gt 0
    $parserUsed = 'n/a'

    $staticSignals = Get-StaticDependencies -Text $text -FilePath $Entry.file
    $dynamicSignals = Get-DynamicDependencies -Text $text

    # For Python files attempt AST-based import extraction (higher accuracy than regex)
    if ($Entry.extension -eq '.py') {
        $astResult = Get-PythonASTImports -FullPath $fullPath
        if ($astResult -and $astResult.parser -eq 'ast') {
            $parserUsed = 'ast'
            # Replace Python import signals with AST results; keep non-Python regex signals
            $nonPythonStatic = New-Object System.Collections.Generic.List[object]
            foreach ($sig in @($staticSignals)) {
                if ($sig.evidence -notmatch 'python') { $nonPythonStatic.Add($sig) }
            }
            foreach ($imp in @($astResult.imports)) {
                if ($imp.Length -gt 0) {
                    $nonPythonStatic.Add([pscustomobject]@{ reference = $imp; confidence = 100; evidence = 'python_ast_import' })
                }
            }
            $staticSignals = [object[]]$nonPythonStatic.ToArray()
            # Merge AST dynamic imports with existing dynamic signals
            $mergedDynamic = New-Object System.Collections.Generic.List[object]
            foreach ($sig in @($dynamicSignals)) { $mergedDynamic.Add($sig) }
            foreach ($dynImp in @($astResult.dynamic_imports)) {
                if ($dynImp.Length -gt 0) {
                    $mergedDynamic.Add([pscustomobject]@{ reference = $dynImp; confidence = 90; evidence = 'python_ast_dynamic_import' })
                }
            }
            $dynamicSignals = [object[]]$mergedDynamic.ToArray()
        }
        else {
            $parserUsed = 'regex_fallback'
            $script:regexFallbackCount++
        }
    }
    ...
    return [ordered]@{
        ...
        structural_exempt  = $isStructuralExempt
        di_wired           = $isDIWired
...
$parserMetricsPath = Join-Path $RUN_OUTPUT_DIR 'parser_metrics.json'
[ordered]@{
    ast_used           = $script:astUsedCount
    regex_fallback_used = $script:regexFallbackCount
    python_available   = ($null -ne $pythonExe)
    python_executable  = $pythonExe
} | ConvertTo-Json | Out-File -LiteralPath $parserMetricsPath -Encoding UTF8
Write-Status "Parser metrics: $parserMetricsPath"
